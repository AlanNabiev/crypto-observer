  ...
  created() {
    ...
    if (tickersData) {
      ...
      this.tickers.forEach((ticker) => {
        this.subscrideToUpdate(ticker.name);
      });
    }
  },

  methods: {
    subscrideToUpdate(tickerName) {
      setInterval(async () => {
        const f = await fetch(
          `https://min-api.cryptocompare.com/data/price?fsym=${tickerName}&tsyms=USD&api_key=64896e4640f0858ed5820b2fca914df4f3d0cb0ad02dc8b20c6446c50e8ceea2`
        );
    ...
    add() {
      ...
      if (this.ticker !== "" && this.validateSel !== false) {
        ...
        this.subscrideToUpdate(currentTicker.name);
